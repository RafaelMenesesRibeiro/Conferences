...
class GradientMapSingleMaximum(GradientMap):
	"""
	...
	maximum_point           : numpy.ndarray
		shape - (2,)
		position of the single maximum point, in this case equal to highest_point
	maximum_point_offset    : numpy.ndarray
		shape - (2,)
		maximum point divided by the PARABOLOID_MULTIPLIER to improve efficiency when offsetting x, y value in order
		to move the paraboloid's center to maximum_point's position
	"""
	...
	def __init__(self, mission_plane: MissionPlane, space_size: float, maximum_point: np.ndarray((2,)) = None):
		"""
		Parameters
		----------
		...
		maximum_point   : numpy.ndarray
			shape - (2,)
			position of the single maximum point
		"""
		if maximum_point is None:
			maximum_point = np.random.random((2,)) * space_size
		self.maximum_point = maximum_point
		self.maximum_point_offset = maximum_point / GradientMapSingleMaximum.PARABOLOID_MULTIPLIER
		super(GradientMapSingleMaximum, self).__init__(mission_plane, space_size)
	...
	def get_value(self, position: np.ndarray((2,))) -> float:
		...
		float
			height value
		"""
		position = position / GradientMapSingleMaximum.PARABOLOID_MULTIPLIER - self.maximum_point_offset
		x = position[0]
		y = position[1]
	...
	def update_utility_value(self, is_update_increasing: bool = True, is_update_maximum: bool = True,
		...
		"""
		raise NotImplementedError("TODO - GradientMapSingleMaximum")
	...
	def translate_rendezvous_area(self, is_in_boundaries_function, is_move_maximum: bool = None,
		...
		"""
		Moves the gradient map's single maximum to a close random position.
		...
		"""
		new_position = self.maximum_point + (np.random.rand(2, ) * 2 - 1)
		if is_in_boundaries_function(new_position):
			self.maximum_point = new_position
			self.maximum_point_offset = self.maximum_point / GradientMapSingleMaximum.PARABOLOID_MULTIPLIER
	...
	def encode(self) -> dict:
		...
		"""
		return {
			'gradient_map_type': GradientMap.GradientMapType.SINGLE_MAX.value,
			'maximum_point': self.maximum_point.tolist()}
